...
async function renderLeaders(limit) {
  limit = limit || 5; // Default to 5 for sidebar
...
// ═══ Render full leaders list for Leaders page (10 items) ═══
async function renderLeadersFull() {
  var rankColors = ['gold', 'silver', 'bronze'];
...
function openLangModal() {
  // Update modal UI to show current language
  var langOptEn = document.getElementById('langOptEn');
  var langOptRu = document.getElementById('langOptRu');
  var langCheckEn = document.getElementById('langCheckEn');
  var langCheckRu = document.getElementById('langCheckRu');
  
  if (langOptEn && langOptRu) {
    langOptEn.classList.remove('selected');
    langOptRu.classList.remove('selected');
  }
  
  if (LANG === 'en') {
    if (langOptEn) langOptEn.classList.add('selected');
  } else if (LANG === 'ru') {
    if (langOptRu) langOptRu.classList.add('selected');
  }
...
function profileHTML(sfx) {
  ...
  return '<div class="phero">'
    ...
    + '<div style="position:relative;width:14px;height:14px;margin-right:8px">'
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><path d="M12 2l3.09 6.26L22 9.27l-5 4.97 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>'
    ...
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0z"/></svg>'
    + '<span style="font-size:0.93rem">Theme Studio</span>'
    ...
    + '<span style="font-size:1rem">' + themeIcon + '</span>'
    + '</div>'
    + '<div class="lang-row" data-lang-toggle="1">'
    ...
    + '<span style="font-size:0.93rem">' + T('language') + '</span>'
    + '<span class="lang-cur" id="lc' + sfx + '">' + LANG.toUpperCase() + '</span>'
    + '</div>'
    + '<div class="sset" onclick="window.toggleFontSize && toggleFontSize()">'
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><path d="M4 7V4h16v3"/><path d="M9 20h6"/><path d="M12 4v16"/></svg>'
    + '<span>' + (window.LANG === 'ru' ? 'Размер шрифта' : 'Font Size') + '</span>'
    + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--mu)"><polyline points="9 18 15 12 9 6"/></svg></div>'
    + '<div class="sset" data-pulse="1" data-settings="notifications">'
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>'
    + '<span>' + T('notifications') + '</span>'
    + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--mu)"><polyline points="9 18 15 12 9 6"/></svg></div>'
    + '<div class="sset" data-pulse="1" data-settings="privacy">'
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>'
    + '<span>' + T('privacy') + '</span>'
    + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--mu)"><polyline points="9 18 15 12 9 6"/></svg></div>'
    + '<div class="sset" data-settings="profile-bg" onclick="window.ProfileEditEngine && ProfileEditEngine.openProfileBg()">'
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>'
    + '<span>' + (window.LANG === 'ru' ? 'Фон профиля' : 'Profile Background') + '</span>'
    + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--mu)"><polyline points="9 18 15 12 9 6"/></svg></div>'
    + '<a class="sset" href="about.html" style="text-decoration:none;color:inherit">'
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>'
    + '<span>' + (typeof T === 'function' && T('aboutUs') !== 'aboutUs' ? T('aboutUs') : (window.LANG === 'ru' ? 'О нас' : 'About us')) + '</span>'
    ...
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>'
    + '<span>' + (window.LANG === 'ru' ? 'Поддержка' : 'Support') + '</span>'
    + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--mu)"><polyline points="9 18 15 12 9 6"/></svg></div>'
    + (PROFILE.is_admin
        ? '<div class="sset" onclick="window.checkUpdates && checkUpdates()">'
          + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><path d="M21 12a9 9 0 0 0-9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M3 12a9 9 0 0 0 9 9 9.75 9.75 0 0 0 6.74-2.74L21 16"/><path d="M16 16h5v5"/></svg>'
          ...
          + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--vl2);margin-right:8px"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>'
          + '<span style="color:var(--vl2)">Панель управления</span>'
          + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--mu)"><polyline points="9 18 15 12 9 6"/></svg></a>'
          + '<div class="sset" onclick="window.quickAccountSwitch && quickAccountSwitch()">'
          + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--ac)"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>'
          + '<span>' + (window.LANG === 'ru' ? 'Быстрая смена аккаунта' : 'Quick Account Switch') + '</span>'
          + '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--mu)"><polyline points="9 18 15 12 9 6"/></svg></div>'
        : '')
    + '<div class="sset" data-logout="1">'
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--red)"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" x2="3" y1="12" y2="12"/></svg>'
...
function renderDailyStreakHTML() {
  ...
  return '<div class="streak-container">'
    ...
    + '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:#e8c55a;margin-right:6px;vertical-align:middle"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>'
    + (window.LANG === 'ru' ? 'Ежедневный стрик' : 'Daily Streak')
    + '</div>'
